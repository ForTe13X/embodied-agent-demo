...
2. **几何重规划是 access-盲的**:改道轨迹**穿过了 r1(受限区)**——因为 r1 的"受限"是拓扑/注册表
   ...
   > **已补(F-01 强制层,post-review)**:上面第 2 条此前只**披露**、未**强制**。现新增运行期访问围栏
   > [`embodied_agent/geofence.py`](../embodied_agent/geofence.py):`TransitGuard` 盯 `RclpyAdapter._cur_node()`
   > 的实际位置流,踏入未授权受限/禁入区即取消 Nav2 目标、安全停,终态上浮 `transit_violation`。
   > 这是**强制/检测**层(不依赖规划器 access-aware);把 r1 涂进 keepout 是互补的**预防**层。
   > 判定逻辑纯函数、宿主 venv 单测(`tests/test_transit_geofence.py`);真实栈端到端见
   > [`smoke_transit_guard.py`](smoke_transit_guard.py)(需容器)。
