...
.\.venv\Scripts\Activate.ps1   # 激活 venv:之后的 `python` 才是 venv 的(否则全局 python 缺 rich/langgraph 等依赖,会 ModuleNotFoundError)
...
验证:`.\.venv\Scripts\python -m pytest tests -q` → **expected output:`65 passed`**。
