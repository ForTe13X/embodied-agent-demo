...
## 6. rclpy 适配器骨架(Phase B,未实现)

```python
class RclpyAdapter:
    """构造时只创建 NavigateToPose ActionClient,不创建任何 publisher。
    附带断言测试:枚举 node 的 publishers,出现 cmd_vel/力矩类 topic 即 fail——
    'LLM 拿不到速度接口'是结构性保证,不是口号。"""
```
